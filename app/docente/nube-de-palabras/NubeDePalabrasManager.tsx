...
// Parte interactiva de /docente/nube-de-palabras. La página (page.tsx) es un
// Server Component: verifica la sesión, trae las nubes del docente y las
// pasa acá — antes esta pantalla las pedía a Firestore desde el navegador
// recién después de cargar, y mostraba "Debes estar logueado" hasta que
// Firebase Auth terminaba de inicializar.
export default function NubeDePalabrasManager({
  teacherId,
  initialSessions,
}: {
  teacherId: string;
  initialSessions: WordCloudSession[];
}): JSX.Element {
  ...
  useEffect(() => {
    if (!loading && user?.uid !== teacherId) router.refresh();
  }, [loading, user?.uid, teacherId, router]);

  const [title, setTitle] = useState('');
  const [creating, setCreating] = useState(false);
  const [error, setError] = useState('');

  const [sessions, setSessions] = useState<WordCloudSession[]>(initialSessions);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const session = await WordCloudService.createSession(teacherId, title);
