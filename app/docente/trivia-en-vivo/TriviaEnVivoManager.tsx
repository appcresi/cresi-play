...
// Parte interactiva de /docente/trivia-en-vivo (crear una partida y ver el
// historial). La página (page.tsx) es un Server Component: verifica la sesión
// y trae las trivias disponibles y las partidas del docente. La sala en vivo
// en sí (`[codigo]`) sigue siendo cliente: usa suscripciones en tiempo real.
export default function TriviaEnVivoManager({
  teacherId,
  initialTrivias,
  initialSessions,
}: {
  teacherId: string;
  initialTrivias: TriviaOption[];
  initialSessions: LiveTriviaSession[];
}): JSX.Element {
  ...
  const [trivias] = useState<TriviaOption[]>(initialTrivias);
  const [selectedTriviaId, setSelectedTriviaId] = useState('');
  const [creating, setCreating] = useState(false);
  const [error, setError] = useState('');
  const [triviaSearch, setTriviaSearch] = useState('');

  const [sessions, setSessions] = useState<LiveTriviaSession[]>(initialSessions);
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      const session = await LiveTriviaService.createSession(teacherId, selectedTriviaId);
