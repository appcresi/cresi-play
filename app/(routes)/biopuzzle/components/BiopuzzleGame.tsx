...
/**
 * El juego de BioPuzzle en sí (tablero + panel lateral + modal de
 * felicitación), separado de la página completa (Biopuzzle.tsx) para
 * poder reusarlo embebido dentro de una tarea (ver BiopuzzleInline.tsx en
 * components/tareas) sin duplicar toda la lógica de arrastrar/soltar.
 */
export function BiopuzzleGame({
  ...
  /** true cuando la tarea liga a un sistema puntual: oculta el selector
   *  de sistema y el botón "Siguiente sistema" — hay un solo desafío. */
  locked?: boolean;
  /** false para uso embebido dentro de una tarea: sin GameStatusBar fijo
   *  ni layout de página completa, solo el tablero + panel. */
  showChrome?: boolean;
  /** false cuando lo previsualiza el docente: el juego funciona igual
   *  pero no toca los puntos/progreso reales de esa cuenta. */
  awardPoints?: boolean;
  /** Se dispara cada vez que se completa un sistema — TareaViewScreen lo
   *  usa para exigir haber resuelto el desafío antes de poder entregar. */
  onComplete?: () => void;
}) {
  const [currentSystemIndex, setCurrentSystemIndex] = useState(initialSystemIndex);
  ...
  useEffect(() => {
    ...
    setUserData(data);
    if (awardPoints) UserDataManager.visitActivity(ACTIVITY_ID);
  ...
  const board = (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
      ...
      <div className="flex flex-col gap-3">
        {/* Selector de sistema (oculto si la tarea ligó uno puntual) */}
  ...
  const completionModal = levelCompleted && (
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-100 dark:border-gray-700 max-w-md w-full p-8">
        <div className="text-center">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={resetCurrentGame}
              className={`px-4 py-2.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-full font-semibold hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${
