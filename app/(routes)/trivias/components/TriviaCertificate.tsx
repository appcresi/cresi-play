...
export default function TriviaCertificate(): JSX.Element {
  ...
  const trivia = searchParams.get("trivia");
  const percentage = searchParams.get("percentage");

  const { toPDF, targetRef } = usePDF({
    method: "save",
    filename: `${name?.replace(' ', '')}-${trivia}-cresi.pdf`,
    page: { margin: Margin.SMALL, orientation: "landscape" }
  })

  if (!name || !trivia || !percentage) {
  ...
  return (
    <>
      ...
      <div className="m-auto bg-primary-dark lg:w-[1100px]" ref={targetRef}>
          ...
          <p className="my-4 text-6xl font-semibold text-primary">{name.toLocaleUpperCase()}</p>
          <p className="my-6 text-xl">Por haber aprobado la trivia <b>"{trivia}"</b>, con un porcentaje del {percentage}% de aciertos.</p>
