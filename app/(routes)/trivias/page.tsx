...
function organizeIndexesByLevel(indexFields: TriviaIndexFields[]): Record<number, TriviaIndexFields[]> {
...
async function getTriviaIndexes(): Promise<TriviaIndexFields[]> {
  const response = await fetch(`${API_URL}/trivias?author=CRESI`, { next: { revalidate: 3600 } })
    ...
    <main className='mx-auto px-4 max-w-5xl'>
      <section className='flex flex-col gap-4 justify-center items-center md:flex-row md:items-center lg:justify-evenly py-8'>
        <span className='md:max-w-sm'>
          <h1 className='my-4 text-4xl font-bold text-primary md:text-5xl'>Trivias</h1>
          <h2 className='text-lg text-gray-600'>
          ¡Es hora de jugar! Aquí encontrarás una variedad de trivias clasificadas por niveles.
          Usa el buscador para encontrar fácilmente la trivia que te interese.
          Además, puedes ajustar el tiempo de juego haciendo clic en el botón de configuración. 
          </h2>
