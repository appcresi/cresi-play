    ...
    <>
      ...
      {hydrated && consent === null && (
        <div className="fixed bottom-0 inset-x-0 z-[9998] p-3 sm:p-4 flex justify-center pointer-events-none">
          <div
            className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl border border-ink/10 p-4 sm:p-5
                     flex flex-col sm:flex-row sm:items-center gap-4 pointer-events-auto"
            role="region"
            aria-label="Aviso de cookies"
          >
            <p className="text-xs sm:text-sm text-ink/70 leading-relaxed flex-1">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="px-4 py-2 rounded-full border border-ink/15 text-ink/70 text-xs sm:text-sm
                         font-semibold hover:bg-cream transition-colors"
              >
                Rechazar
              </button>
              <button
