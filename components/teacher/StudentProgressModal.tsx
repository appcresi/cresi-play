        ...
        <div className="flex items-center justify-between p-5 border-b border-pink-light">
          <div className="flex items-center gap-3">
            <Image
              ...
              width={40}
              height={40}
              className="w-10 h-10 rounded-full object-cover border border-pink-light"
            />
            <div>
              <h2 className="text-base font-bold text-ink">{student.username}</h2>
            ...
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-cream rounded-lg p-3 text-center">
                  ...
                  <p className="text-sm font-bold text-ink">{progress.totalScore}</p>
                  <p className="text-[10px] text-ink/60">Puntos</p>
                </div>
                <div className="bg-cream rounded-lg p-3 text-center">
                  <IconFlame className="w-4 h-4 text-orange-500 mx-auto mb-1" />
                  <p className="text-sm font-bold text-ink">{progress.streak}</p>
                  <p className="text-[10px] text-ink/60">Racha (días)</p>
                </div>
                <div className="bg-cream rounded-lg p-3 text-center">
                  <IconCalendarTime className="w-4 h-4 text-blue-500 mx-auto mb-1" />
                  <p className="text-xs font-bold text-ink">{formatDate(progress.lastActive)}</p>
                  <p className="text-[10px] text-ink/60">Última vez</p>
                </div>
              </div>

              <div>
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs font-medium text-ink/80">Actividades completadas</p>
                  <p className="text-xs text-ink/60">{completedCount}/{TOTAL_ACTIVITIES}</p>
                </div>
              ...
              <div>
                <p className="text-xs font-medium text-ink/80 mb-2">Detalle</p>
                {progress.completedActivities.length === 0 ? (
                  <p className="text-xs text-ink/40">Todavía no completó actividades.</p>
                ) : (
                  <ul className="space-y-1.5">
                    {progress.completedActivities.map((title) => (
                      <li
                        key={title}
                        className="flex items-center justify-between text-sm text-ink/80 bg-cream rounded-lg px-3 py-2"
                      >
                        <span>{title}</span>
