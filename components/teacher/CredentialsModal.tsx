    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-sm w-full p-5">
        <div className="flex items-center justify-between mb-1">
          <h3 className="font-bold text-ink dark:text-gray-100 flex items-center gap-2">
        ...
        <p className="text-xs text-ink/60 dark:text-gray-400 mb-4">
          Esto es lo que el alumno necesita para iniciar sesión en esta clase.
        </p>

        <div className="space-y-3">
          <div>
            <label className="block text-xs font-medium text-ink/80 dark:text-gray-300 mb-1">Usuario</label>
            <input
              type="text"
              value={username}
              onChange={(e) => { setUsername(e.target.value); setError(''); }}
              disabled={saving || resetting}
              className="w-full px-3 py-2 border border-pink-light dark:border-gray-700 dark:bg-gray-700 dark:text-gray-100 rounded-lg text-sm
                       focus:outline-none focus:ring-2 focus:ring-coral disabled:opacity-50"
            />
          </div>

          <div>
            <label className="block text-xs font-medium text-ink/80 dark:text-gray-300 mb-1">Contraseña</label>
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-ink/40 dark:text-gray-500 hover:text-ink/70 dark:hover:text-gray-300"
        ...
        {target.claimedUid && (
          <div className="mt-4 pt-4 border-t border-pink-light dark:border-gray-700">
            {!confirmingReset ? (
              <button
              ...
              <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-lg p-3">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={resetting}
                    className="px-3 py-1.5 text-xs text-ink/70 dark:text-gray-400 hover:bg-pink-light dark:hover:bg-gray-700 rounded-lg disabled:opacity-50"
                  >
                    Cancelar
                  </button>
                  <button
