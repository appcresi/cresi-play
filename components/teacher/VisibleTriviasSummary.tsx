...
export const VisibleTriviasSummary = ({ classroom, teacherId }: { classroom: Classroom; teacherId: string }) => {
  ...
  if (trivias === null) {
    return (
      <div className="flex items-center gap-2 text-ink/40 dark:text-gray-500 text-xs py-2">
  ...
  if (trivias.length === 0) {
    return <p className="text-xs text-ink/40 dark:text-gray-500">Todavía no hay trivias seleccionadas.</p>;
  }
      ...
      {trivias.map((t) => {
        ...
        return (
          ...
          >
            ...
            <p className="text-xs font-semibold text-ink dark:text-gray-100 leading-tight mb-0.5 line-clamp-2 break-words">{t.name}</p>
