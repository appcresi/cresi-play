    ...
    <div>
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Asigná consignas, ligadas o no a una actividad, con fecha de entrega y puntos.</p>
        </div>
        <button
          onClick={() => setShowCreateModal(true)}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-full text-xs font-medium transition-colors shrink-0"
        >
      ...
      {loading ? (
        ...
        <p className="text-sm text-gray-400 text-center py-12">Todavía no creaste ninguna tarea.</p>
      ) : (
        <ul className="divide-y divide-gray-100 border border-gray-200 rounded-lg overflow-hidden">
          {tareas.map((tarea) => (
            <li key={tarea.id} className="p-4 hover:bg-gray-50 transition-colors">
              <div className="flex items-start justify-between gap-3">
                <button onClick={() => onOpenTarea(tarea)} className="flex items-start gap-3 flex-1 min-w-0 text-left">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-gray-900 truncate">{tarea.title}</p>
                    <p className="text-xs text-gray-500 mt-0.5 line-clamp-1">{tarea.consigna}</p>
                    <div className="flex flex-wrap items-center gap-2 mt-1.5">
                      ...
                      <span className="text-[11px] text-gray-500">{tarea.points} pts</span>
                      <span className={`text-[11px] ${isPastDue(tarea.dueDate) ? 'text-red-500' : 'text-gray-500'}`}>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-5">
            <h3 className="font-bold text-gray-900 mb-1">¿Eliminar esta tarea?</h3>
