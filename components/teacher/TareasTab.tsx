  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          <h3 className="text-sm font-medium text-ink dark:text-gray-100">Tareas</h3>
      ...
      {loading ? (
        ...
        <p className="text-sm text-ink/40 dark:text-gray-500 text-center py-12">Todavía no creaste ninguna tarea.</p>
      ) : (
        <ul className="divide-y divide-pink-light dark:divide-gray-700 border border-pink-light dark:border-gray-700 rounded-lg overflow-hidden">
          {tareas.map((tarea) => (
              ...
              <div className="flex items-start justify-between gap-3">
                <button onClick={() => onOpenTarea(tarea)} className="flex items-start gap-3 flex-1 min-w-0 text-left">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-ink dark:text-gray-100 truncate">{tarea.title}</p>
                    <p className="text-xs text-ink/60 dark:text-gray-400 mt-0.5 line-clamp-1">{tarea.consigna}</p>
                    <div className="flex flex-wrap items-center gap-2 mt-1.5">
                      <span className="inline-block px-2 py-0.5 bg-pink-light dark:bg-gray-700 text-ink/70 dark:text-gray-300 text-[11px] rounded-full">
                        {LINKED_TYPE_LABELS[tarea.linkedActivity.type]}
                        {tarea.linkedActivity.label ? `: ${tarea.linkedActivity.label}` : ''}
                      </span>
                      <span className="text-[11px] text-ink/60 dark:text-gray-400">{tarea.points} pts</span>
                      <span className={`text-[11px] ${isPastDue(tarea.dueDate) ? 'text-red-500' : 'text-ink/60 dark:text-gray-400'}`}>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button
                    ...
                    className="p-1.5 text-ink/40 dark:text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-full transition"
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-sm w-full p-5">
            <h3 className="font-bold text-ink dark:text-gray-100 mb-1">¿Eliminar esta tarea?</h3>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setDeleteTarget(null)} className="px-4 py-2 text-sm text-ink/70 dark:text-gray-400 hover:bg-pink-light dark:hover:bg-gray-700 rounded-lg">
                Cancelar
              </button>
