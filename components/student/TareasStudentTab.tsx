  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="px-5 py-3 border-b border-gray-100">
        <h2 className="text-sm font-semibold text-gray-700">Tareas asignadas</h2>
      </div>
      ...
      {loading ? (
        ...
        <div className="p-8 text-center text-sm text-gray-400">Tu docente todavía no asignó ninguna tarea.</div>
      ) : (
        <ul className="divide-y divide-gray-100">
          {tareas.map((t) => {
            ...
            if (entrega?.status === 'calificada') {
              ...
              statusBadge = (
                <span className={`flex items-center gap-1 text-xs font-medium ${isPastDue ? 'text-red-500' : 'text-gray-400'}`}>
            ...
            return (
              <li key={t.id}>
                <button onClick={() => setOpenTarea(t)} className="w-full flex items-start gap-3 px-5 py-4 hover:bg-gray-50 transition-colors text-left">
                  <div className="w-9 h-9 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0">
                    <IconClipboardList className="w-4.5 h-4.5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900">{t.title}</p>
                    <p className="text-xs text-gray-500 mt-0.5 line-clamp-1">{t.consigna}</p>
        ...
        <div className="p-5 space-y-4">
          ...
          {href && (
            <Link
              href={href}
              target="_blank"
              className="inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-50 text-indigo-700 rounded-full text-sm font-semibold hover:bg-indigo-100 transition-colors"
            >
          ...
          {alreadyGraded ? (
            ...
            <>
              <div>
                <label className="block text-sm font-semibold text-gray-900 mb-1">Tu respuesta (opcional)</label>
                <textarea
                  value={responseText}
              ...
              <button
                ...
                disabled={saving}
                className="w-full px-5 py-2.5 bg-indigo-600 text-white rounded-full text-sm font-semibold hover:bg-indigo-700 disabled:opacity-50 flex items-center justify-center gap-2"
              >
