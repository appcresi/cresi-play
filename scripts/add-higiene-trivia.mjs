/**
 * scripts/add-higiene-trivia.mjs
 *
 * Crea una trivia nueva ("Higiene") en la colección `trivia`, con
 * author: 'CRESI' y userId: CRESI_UID, igual que las trivias que se crean
 * desde el panel de administración.
 *
 * El set original tenía 31 preguntas; se revisó el contenido y se dejaron
 * 25, sacando las que quedaban redundantes con otra pregunta del mismo set
 * (mismo dato explicado dos veces):
 *   - "¿Qué debe hacerse con el prepucio durante la higiene?" — repetía lo
 *     que ya explica la pregunta sobre la higiene del pene.
 *   - "¿Qué puede provocar el uso excesivo de productos perfumados...?" —
 *     redundante con la de higiene genital externa y la del jabón
 *     antibacterial (las tres decían "no usar productos perfumados/fuertes").
 *   - "¿Cuál es una buena práctica relacionada con la higiene de las
 *     manos?" — lavado de manos genérico, no específico del eje de
 *     higiene íntima/menstrual del resto del set.
 *   - "¿Qué problemas pueden aparecer al afeitarse de manera incorrecta?"
 *     — tercera pregunta sobre afeitado (ya había dos: compartir máquinas,
 *     no es obligatorio depilarse).
 *   - "¿Una persona puede tener una infección aunque mantenga una buena
 *     higiene?" — mismo mensaje que "la higiene no reemplaza al
 *     preservativo para prevenir ITS".
 *   - "¿Cuál es una característica de una buena higiene genital?" —
 *     resumen genérico de varias preguntas anteriores, y su distractor
 *     ("limpiar la zona interna de la vagina a diario") repetía la
 *     pregunta sobre duchas vaginales.
 * El resto se revisó por precisión médica, distractores no repetidos
 * dentro de la misma pregunta y que la respuesta correcta no apareciera
 * también como distractor — no se encontraron problemas.
 *
 ...
 *        node scripts/add-higiene-trivia.mjs
 *   2. Si se ve bien, subir de verdad:
 *        node scripts/add-higiene-trivia.mjs --apply
 */
...
// Mismo uid que lib/constants.ts (CRESI_UID) en appcresi-admin: es el uid
// que usa isCresiTrivia() para reconocer el contenido oficial.
const CRESI_UID = 'feHhnkE3m1Yzbwn4AgEA7rJjsul1';

const TRIVIA_NAME = 'Higiene';
const TRIVIA_LEVEL = 1;
...
async function main() {
  ...
  console.log(`Trivia: "${TRIVIA_NAME}" — nivel ${TRIVIA_LEVEL} — ${questions.length} preguntas (de 31 originales, se sacaron 6 por redundancia)\n`);
  ...
  if (APPLY) {
    ...
    console.log('  node scripts/add-higiene-trivia.mjs --apply');
  }
  process.exit(0);
}
